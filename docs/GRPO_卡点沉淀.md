...
- **Reward**：`src/sweforge/reward/verifier.py`（CleanVerifier，git apply + build + test 真实执行）+ `schemas/verification.py` 的 `binary_reward`（1.0 if resolved else 0.0）——**reward 数值是用户的，advantage 计算是 verl 的**。
...
| reward_score = 1.0/0.0 | 我们（CleanVerifier + binary_reward） |
